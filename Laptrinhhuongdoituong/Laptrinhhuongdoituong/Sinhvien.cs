    ...
    class Sinhvien
    {
           ...
           
       {
        ...
        public void laygiatri( int stt)
        {
            ...
            sv.dangkynv(stt);
            sbd = sv.Sbd;
            sbd = sbd.Trim('"'); /// trim là hàm cắt kí tự
            kv = sv.Khuvuc;
            uutien = sv.Uutien;
            dantoc = sv.Dantoc;
            ten = sv.Ten;
            ...
            for (int i = 0; i < 4; i++)
                for (int j = 0; j < 2; j++)
                    nguyenvong[i, j] = sv.Nguyenvong[i, j];///gán nguyện vọng bên sinhvien vào nguyện vọng file docdulieu
       ...
       private double diemcacmon( string str)
        ...
        {
            SQLiteConnection conn = new SQLiteConnection(@"Data Source = C:/Users/Thaibinh/Desktop/SQLite-thuvienwinform/SQLite-thuvienwinform/Trần Ngọc Hoàn");
            conn.Open();
